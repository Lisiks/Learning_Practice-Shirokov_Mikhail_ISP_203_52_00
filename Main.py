...
class MyApplication:
    ...
    def level_exit(self, item):
        while self.Game_active_bomb_list:
            self.Game_active_bomb_list.pop().cancel()
    ...
    def play_game_level(self, level_number):
        ...
        threading.Thread(name='Check_player_colliderect', target=self.check_enemy_colliderect, daemon=True).start()
        ...
        match self.Game_level:
            case 1:
                self.Game_enemy_list.extend(
                    [Enemy(images['animation']['slime'], self.Game_canvas, 0, 5, 'Sounds/Slime_dead.mp3', 0.06, 0.2, 'Enemy'),
                     Enemy(images['animation']['slime'], self.Game_canvas, 0, 11, 'Sounds/Slime_dead.mp3', 0.06, 0.2, 'Enemy'),
                     Enemy(images['animation']['slime'], self.Game_canvas, 5, 11, 'Sounds/Slime_dead.mp3', 0.06, 0.2, 'Enemy'),
                     Enemy(images['animation']['slime'], self.Game_canvas, 3, 16, 'Sounds/Slime_dead.mp3', 0.06, 0.2, 'Enemy')])
            case 2:
                self.Game_enemy_list.extend(
                    [Enemy(images['animation']['green_slime'], self.Game_canvas, 1, 4, 'Sounds/Slime_dead.mp3', 0.06, 0.17, 'Enemy'),
                     Enemy(images['animation']['green_slime'], self.Game_canvas, 1, 10, 'Sounds/Slime_dead.mp3', 0.06, 0.17, 'Enemy'),
                     Enemy(images['animation']['green_slime'], self.Game_canvas, 1, 16, 'Sounds/Slime_dead.mp3', 0.06, 0.17, 'Enemy'),
                     Enemy(images['animation']['skeleton'], self.Game_canvas, 4, 7, 'Sounds/Skeleton_dead.mp3', 0.06, 0.07, 'Enemy'),
                     Enemy(images['animation']['skeleton'], self.Game_canvas, 5, 13, 'Sounds/Skeleton_dead.mp3', 0.06, 0.07, 'Enemy')])
            case 3:
                self.Game_enemy_list.extend(
                    [Enemy(images['animation']['zombie'], self.Game_canvas, 3, 1, 'Sounds/Zombie_dead.mp3', 0.05, 0.13, 'Enemy'),
                     Enemy(images['animation']['zombie'], self.Game_canvas, 0, 9, 'Sounds/Zombie_dead.mp3', 0.05, 0.13, 'Enemy'),
                     Enemy(images['animation']['zombie'], self.Game_canvas, 5, 6, 'Sounds/Zombie_dead.mp3', 0.05, 0.13, 'Enemy'),
                     Enemy(images['animation']['zombie'], self.Game_canvas, 3, 16, 'Sounds/Zombie_dead.mp3', 0.05, 0.13, 'Enemy'),
                     Enemy(images['animation']['green_slime'], self.Game_canvas, 0, 6, 'Sounds/Slime_dead.mp3', 0.06, 0.17, 'Enemy'),
                     Enemy(images['animation']['green_slime'], self.Game_canvas, 5, 9, 'Sounds/Slime_dead.mp3', 0.06, 0.17, 'Enemy'),
                     Enemy(images['animation']['green_slime'], self.Game_canvas, 3, 15, 'Sounds/Slime_dead.mp3', 0.06, 0.17, 'Enemy')])
            case 4:
                self.Game_enemy_list.extend(
                    [Enemy(images['animation']['slime'], self.Game_canvas, 4, 0, 'Sounds/Slime_dead.mp3', 0.06, 0.2, 'Enemy'),
                     Enemy(images['animation']['slime'], self.Game_canvas, 3, 16, 'Sounds/Slime_dead.mp3', 0.06, 0.2, 'Enemy'),
                     Enemy(images['animation']['slime'], self.Game_canvas, 5, 9, 'Sounds/Slime_dead.mp3', 0.06, 0.2, 'Enemy'),
                     Enemy(images['animation']['zombie'], self.Game_canvas, 2, 3, 'Sounds/Zombie_dead.mp3',   0.05, 0.13, 'Enemy'),
                     Enemy(images['animation']['zombie'], self.Game_canvas, 0, 13, 'Sounds/Zombie_dead.mp3',  0.05, 0.13, 'Enemy'),
                     Enemy(images['animation']['skeleton'], self.Game_canvas, 3, 7, 'Sounds/Skeleton_dead.mp3', 0.06, 0.07, 'Enemy'),
                     Enemy(images['animation']['skeleton'], self.Game_canvas, 5, 8, 'Sounds/Skeleton_dead.mp3', 0.06, 0.07, 'Enemy')])
            case 5:
                self.Game_enemy_list.extend(
                    [Enemy(images['animation']['lava_slime'], self.Game_canvas, 4, 0, 'Sounds/Slime_dead.mp3', 0.06, 0.1, 'Enemy'),
                     Enemy(images['animation']['lava_slime'], self.Game_canvas, 0, 4, 'Sounds/Slime_dead.mp3', 0.06, 0.1, 'Enemy'),
                     Enemy(images['animation']['lava_slime'], self.Game_canvas, 4, 16, 'Sounds/Slime_dead.mp3', 0.06, 0.1, 'Enemy'),
                     Enemy(images['animation']['lava_slime'], self.Game_canvas, 5, 12, 'Sounds/Slime_dead.mp3',  0.06, 0.1, 'Enemy'),
                     Enemy(images['animation']['wither'], self.Game_canvas, 3, 6, 'Sounds/Skeleton_dead.mp3', 0.06, 0.05, 'Enemy'),
                     Enemy(images['animation']['wither'], self.Game_canvas, 3, 8, 'Sounds/Skeleton_dead.mp3', 0.06, 0.05, 'Enemy'),
                     Enemy(images['animation']['wither'], self.Game_canvas, 3, 10, 'Sounds/Skeleton_dead.mp3', 0.06, 0.05, 'Enemy')])

        for game_enemy in self.Game_enemy_list:
            enemy_i, enemy_j = game_enemy.get_mpos()
            game_enemy.entity_pack(*self.Game_matrix[enemy_i][enemy_j][0])
            threading.Thread(target=self.animate_thread_function, args=(game_enemy,), daemon=True).start()

    def animate_thread_function(self, game_enemy):
        while game_enemy in self.Game_enemy_list:
            enemy_i, enemy_j = game_enemy.get_mpos()
            correct_moves = list()
            ...
            if self.move_is_correct(enemy_i + 1, enemy_j):
                correct_moves.append((100, 'y', 'i'))
            game_enemy.enemy_moving(*random.choice(correct_moves))

    def check_enemy_colliderect(self):
        while self.Game_enemy_list:
            player_x, player_y = self.Game_player.get_cords()
            for canvas_object in self.Game_canvas.find_overlapping(player_x - 20, player_y-40, player_x + 20, player_y + 40):
                if 'Enemy' in self.Game_canvas.gettags(canvas_object):
    ...
    def player_action(self, event):
        ...
        if (event.keycode == self.Key_right) and self.move_is_correct(player_i, player_j + 1) and not self.Game_player.is_on_moving():
            threading.Thread(target=self.Game_player.player_moving, args=(100, 'x', 'j', 'right'), daemon=True).start()
            self.check_player_position()

        elif (event.keycode == self.Key_left) and self.move_is_correct(player_i, player_j - 1) and not self.Game_player.is_on_moving():
            threading.Thread(target=self.Game_player.player_moving, args=(-100, 'x', 'j', 'left'), daemon=True).start()
            self.check_player_position()

        elif (event.keycode == self.Key_up) and self.move_is_correct(player_i - 1, player_j) and not self.Game_player.is_on_moving():
            threading.Thread(target=self.Game_player.player_moving, args=(-100, 'y', 'i'), daemon=True).start()
            self.check_player_position()

        elif (event.keycode == self.Key_down) and self.move_is_correct(player_i + 1, player_j) and not self.Game_player.is_on_moving():
            threading.Thread(target=self.Game_player.player_moving, args=(100, 'y', 'i'), daemon=True).start()
            self.check_player_position()

        elif (event.keycode == self.Key_bomb) and (len(self.Game_active_bomb_list) < self.Game_player.get_characters('max_bomb')):
            bomb_image_id = self.Game_canvas.create_image(
                *self.Game_matrix[player_i][player_j][0], anchor='center', image=images['bomb'], tag='FEL')

            self.Game_active_bomb_list.append(threading.Timer(1.2, self.explotions_game_bomb, args=(player_i, player_j, bomb_image_id, self.Game_level)))
            self.Game_active_bomb_list[-1].daemon = True
            self.Game_active_bomb_list[-1].start()
    ...
    def explotions_game_bomb(self, bomb_i, bomb_j, bomb_id, game_level):
        def kill_enemy_function(explotions_i, explotions_j):
            nonlocal player_has_hit, score_point, game_level
            expl_x, expl_y = self.Game_matrix[explotions_i][explotions_j][0]
            if self.Game_player.collidirect(expl_x, expl_y, ranges=60) and not player_has_hit and (game_level == self.Game_level):
                self.hit_game_player()
                player_has_hit = True

            enemy_index = 0
            while enemy_index < len(self.Game_enemy_list):
                if game_level != self.Game_level:
                    return 0
                elif self.Game_enemy_list[enemy_index].collidirect(expl_x, expl_y, ranges=60):
        ...
        self.Game_canvas.delete(bomb_id)
        ...
        for i in range(bomb_i, bomb_i + bomb_range + 1):
            if self.Game_level != game_level:
                return 0
            elif (i > 5) or (i < 0) or (self.Game_matrix[i][bomb_j][2] == 'undestroyable'):
                break
        ...
        for j in range(bomb_j, bomb_j + bomb_range + 1):
            if self.Game_level != game_level:
                return 0
            elif (j > 16) or (j < 0) or (self.Game_matrix[bomb_i][j][2] == 'undestroyable'):
                break
    ...
    def load_end_screen(self, result):
        ...
        self.End_game_head_image = self.End_game_canvas.create_image(self.W // 2 + 50, self.H // 2 - 120,
                                                                     anchor='center', image=images['interface'][result])
